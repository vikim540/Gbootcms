...
func (bc *BaseController) BatchSort(c *gin.Context, modelPtr interface{}, sortColumn string, defaultSort int) {
	...
	if updated == 0 {
		bc.JSONOKMsg(c, "排序未變化 ("+strconv.Itoa(unchanged)+" 条)")
		return
	}
	if unchanged > 0 {
		bc.JSONOKMsg(c, "排序已保存 ("+strconv.Itoa(updated)+" 条，已變更 "+strconv.Itoa(unchanged)+" 条無變化)")
		return
	}
	bc.JSONOKMsg(c, "排序已保存 ("+strconv.Itoa(updated)+" 条)")
}
