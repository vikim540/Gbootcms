...
// validAcodes 快取合法的 acode 列表（啟動時從 DB 載入）
var (
	validAcodes     map[string]bool
	validAcodesOnce sync.Once
)

func isValidAcode(acode string) bool {
	validAcodesOnce.Do(func() {
		validAcodes = make(map[string]bool)
		skipCtx := acodeplugin.SkipAcode(context.Background())
		var areas []model.Area
		model.DB.WithContext(skipCtx).Find(&areas)
		for _, a := range areas {
			validAcodes[a.Acode] = true
		}
	})
	return validAcodes[acode]
}
